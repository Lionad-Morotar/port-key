...
Или если нужен сервер stdio MCP:
...
- `-d, --digits <count>`: желаемое количество цифр для порта (4 или 5, по умолчанию: 4)
- `--padding-zero <true|false>`: добавлять ноль в начале коротких портов (по умолчанию: true). например, `"air"` → 1840
...
Примечания:
- Язык вывода по умолчанию — `cn`. Используйте `--lang en` для отображения сообщений на английском.
...
```json
{
  // Предпочтительное количество цифр для порта (4 или 5)
  "preferDigitCount": 5,
  // Добавлять ноль в начале коротких портов (по умолчанию: true)
  "paddingZero": true,
  // Пользовательское сопоставление букв к цифрам
...
- Этот репозиторий использует монорепозиторий pnpm; основной пакет находится в `packages/core`。
